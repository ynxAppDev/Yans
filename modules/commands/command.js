...
const loadCommand = function ({ moduleList, threadID, messageID }) {
    ...
    for (const nameModule of moduleList) {
        try {
            ...
            if (
                ...
                logger.loader(
                    "Successfully downloaded the entire package for the module" +
                        command.config.name,
                );
            }
            if (
                ...
                try {
                    for (const [key, value] of Object["entries"](
                        ...
                        if (
                            typeof configValue[command.config.name] == undefined
                        )
                    ...
                    throw new Error(
                        "Failed to load config module, error: " +
    ...
    if (errorList.length != 0)
        api.sendMessage(
            "Modules that had problems loading: " + errorList.join(" "),
            threadID,
            messageID,
        );
    api.sendMessage(
...
module.exports.run = function ({ event, args, api }) {
    ...
    switch (args[0]) {
        ...
        case "info": {
            ...
            if (!command)
                return api.sendMessage(
                    "The module you entered does not exist!",
                    threadID,
                    messageID,
                );
